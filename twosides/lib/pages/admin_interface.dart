...
class _AddPictureBoxState extends ConsumerState<AddPictureBox> {
  ...
  Widget build(BuildContext context) {
    final TextEditingController _roleController = TextEditingController(text: "gallery");
    ...
    return Padding(
      ...
      child: Column(
        children: [
          ...
          ElevatedButton(
            onPressed: _selectedFile != null ? () async {
                final XFile x_file = XFile.fromData(
                _selectedFile!.bytes!,
                name: _selectedFile!.name,
              );
              ref.read(adminInterfaceViewModelProvider.notifier).uploadAssetArtist(widget.artist.id, x_file, _roleController.text!);
            } : null,
    ...
    return Padding(
      ...
      child: Column(
        children: [
          ...
          ElevatedButton(
            onPressed: () async {
              await ref.read(adminInterfaceViewModelProvider.notifier).deleteAsset(widget.asset.id);
            },
            style: ElevatedButton.styleFrom(
              shape: RoundedRectangleBorder(
                borderRadius: BorderRadius.circular(100),
...
class AddLiveBox extends ConsumerStatefulWidget {
  ...
  final TextEditingController _newLinkController = TextEditingController(text: "");
  final TextEditingController _newLinkTypeController = TextEditingController(text: "ytb_live");
  final Artist artist;
          ...
          ElevatedButton(
            onPressed: () async {
              if (widget._newLinkController.text != null) {
                ...
                await ref.read(adminInterfaceViewModelProvider.notifier).upsertLinkArtist(
                  "live",
...
class _AddArtistTileState extends ConsumerState<AddArtistTile> {
  ...
  @override
  Widget build(BuildContext context) {
    final TextEditingController _nameController = TextEditingController(text: "Nom Artist");
        final TextEditingController _styleController = TextEditingController(text: "Electro");
    return Row(
      children: [
        SizedBox(
        ...
        ElevatedButton(
          onPressed: () async {
            if (_nameController.text != "Nom Artist")
              await ref.read(adminInterfaceViewModelProvider.notifier).createArtist(_nameController.text, _styleController.text);
          },
          style: ElevatedButton.styleFrom(
            shape: RoundedRectangleBorder(
              borderRadius: BorderRadius.circular(100),
...
class ArtistInfoField extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<ConsumerStatefulWidget> createState() => _ArtistInfoFieldState();
}
...
class _ArtistInfoFieldState extends ConsumerState<ArtistInfoField> {
  ...
  List<TextEditingController> setupLinkController() {
    ...
    for (final linkType in LinkType.values) {
      if (linkType != LinkType.live && linkType != LinkType.release) {
        linkController.add(TextEditingController(text: (linkType.toString()).split('.')[1]));
      }
    }
    for (final link in widget.artist.links!) {
      if (link.type == LinkType.live || link.type == LinkType.release) {
        ...
        final index = linkController.indexWhere((controller) => controller.text == (link.type.toString()).split('.')[1]);
        if (index != -1) {
  ...
  Widget build(BuildContext context) {
    ...
    final TextEditingController _nameController = TextEditingController(text: widget.artist.name);
    final TextEditingController _locationController = TextEditingController(text: widget.artist.location ?? "");
    final TextEditingController _descriptionController = TextEditingController(text: widget.artist.description ?? "");
    final TextEditingController _styleController = TextEditingController(text: widget.artist.style ?? "Electro");
    final List<TextEditingController> _pictureRoleControllers = widget.artist.assets!.map((asset) => TextEditingController(text: assetRoleToString(asset.role))).toList();
    ...
    List<bool> linkHaveChanged = List.filled(_linkControllers.length, false, growable: false);
    ...
    bool hasValuesChanged() {
      if(_originalName != _nameController.text) {return true;}
      if(_originalLocation != _locationController.text) {return true;}
      if(_originalDescription != _descriptionController.text) {return true;}
      if(_originalStyle != _styleController.text) {return true;}
      return false;
    }
                      ...
                      child: Row(
                        children: [
                          ...
                          ElevatedButton(
                            onPressed: () async {
                              if (widget.artist.links != null) {
                                final int linkIndex = widget.artist.links!.indexWhere((link) => link.type == linkType);
                                if (widget.artist.links![linkIndex].url != linkType.toString().split('.')[1]) {
                                  await ref.read(adminInterfaceViewModelProvider.notifier).deleteLink(widget.artist.links![linkIndex].id);
                                }
                              }
                            },
                            style: ElevatedButton.styleFrom(
                ...
                child: ListView(
                  ...
                  children: [
                    for (final (index, asset) in widget.artist.assets!.indexed)
                      PictureBox(asset: asset, roleController: _pictureRoleControllers[index]),
                    ...
                    children: [
                      for (final link in widget.artist.links!)
                        if (link.type == LinkType.live)
                          Builder(
                            builder: (context) {
                              int index = _linkControllers.indexWhere((controller) => controller.text == link.url);
                              if (index != -1) {
                                return Padding(
                                      ...
                                      Row(
                                        children: [
                                          ...
                                          ElevatedButton(
                                            onPressed: () async {
                                              if (widget.artist.links != null) {
                                                final int linkIndex = widget.artist.links!.indexWhere((link) => link.url == _linkControllers[index].text);
                                                await ref.read(adminInterfaceViewModelProvider.notifier).deleteLink(widget.artist.links![linkIndex].id);
                                              }
                                            },
                                            style: ElevatedButton.styleFrom(
              ...
              onPressed: () async {
                if (hasValuesChanged()) {
                  Artist updatedArtist = Artist(
                    ...
                    await ref.read(adminInterfaceViewModelProvider.notifier).updateArtist(updatedArtist);
                }
...
class _ArtistInfoTileState extends State<ArtistInfoTile> {
  ...
  Widget build(BuildContext context) {
    if (widget.artist.assets != null) {
      _assetId = widget.artist.assets!.indexWhere((asset) => asset.role == AssetRole.primary).toString();
    }
        ...
        builder: (context) {
          if (_isOpened) {
            return Container(
              ...
              color: widget.artist.style == "Dub" ? TwoSidesColors.primaryColor.withOpacity(0.1) : TwoSidesColors.secondaryColor.withOpacity(0.2),
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Row(
        ...
        children: [
          TextButton(
            child: Text(
              ...
              style: TextStyle(
                fontSize: 25,
                color: adminInterfaceViewState.pageType.value == AdminInterfacePageTypes.artist ? TwoSidesColors.primaryColor : TwoSidesColors.textColor),
                //fontWeight: adminInterfaceViewState.pageType.value == AdminInterfacePageTypes.artist ? FontWeight.bold : FontWeight.normal,
            ),
            onPressed: () {
              ref.read(adminInterfaceViewModelProvider.notifier).changeInterfacePage(AdminInterfacePageTypes.artist);
            },
          ),
          TextButton(
            child: Text(
              ...
              style: TextStyle(
                fontSize: 25,
                color: adminInterfaceViewState.pageType.value == AdminInterfacePageTypes.event ? TwoSidesColors.primaryColor : TwoSidesColors.textColor),
                //adminInterfaceViewState.pageType.value == AdminInterfacePageTypes.event ? FontWeight.bold : FontWeight.normal,
            ),
            onPressed: () {
              ref.read(adminInterfaceViewModelProvider.notifier).changeInterfacePage(AdminInterfacePageTypes.event);
            },
          ),
          TextButton(
            child: Text(
              ...
              style: TextStyle(
                fontSize: 25,
                color: adminInterfaceViewState.pageType.value == AdminInterfacePageTypes.about ? TwoSidesColors.primaryColor : TwoSidesColors.textColor),
                //adminInterfaceViewState.pageType.value == AdminInterfacePageTypes.about ? FontWeight.bold : FontWeight.normal,
            ),
            onPressed: () {
              ref.read(adminInterfaceViewModelProvider.notifier).changeInterfacePage(AdminInterfacePageTypes.about);
            },
          ),
          ElevatedButton(
            onPressed: () async {
